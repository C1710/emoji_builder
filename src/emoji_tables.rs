...
/// use emoji_builder::emoji_tables::build_table;
...
/// let paths = vec![path];
///
/// let table = build_table(&paths).unwrap();
///
...
/// ```
pub fn build_table(paths: &[PathBuf]) -> Result<EmojiTable, Error> {
...
/// assert_eq!(table.get(&rainbow).unwrap(), rainbow_entry);
/// ```
pub fn expand(table: &mut EmojiTable, path: &Path) -> Result<(), Error> {
    lazy_static! {
...
fn parse_range(table: &EmojiTable, start: &str, end: &str, kind: EmojiKind) -> EmojiTable {
    ...
    for codepoint in start..=end {
        ...
        let existing_kinds = table.get(&codepoint);
        ...
        let existing_description = get_description(table, &codepoint);
...
fn parse_sequence(table: &EmojiTable, emoji: &str, kind: EmojiKind, description: Option<&str>) -> (EmojiTableKey, (Vec<EmojiKind>, Option<String>)) {
    ...
    // Reallocation would be necessary anyway (because of the extension of the vector)
    let existing_kinds = table.get(&code_sequences);
    let mut kinds = match existing_kinds {
        Some((kinds, _)) => {
            let mut new_kinds = Vec::with_capacity(existing_kinds.unwrap().0.len() + 1);
            new_kinds.extend_from_slice(&kinds);
    ...
    let existing_description = get_description(table, &code_sequences);
